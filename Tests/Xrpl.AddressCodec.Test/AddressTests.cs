...
namespace Xrpl.AddressCodec.Tests
{
    ...
    public class TestUAddressCodec
    {
        ...
        public void InvalidXAddressF()
        {
            ...
            JsonArray testCases = JsonNode.Parse(_testCases)!.AsArray();

            // MAIN
            for (int i = 0; i < testCases.Count; i++)
            {
                JsonArray testCase = testCases[i]!.AsArray();
                string classicAddress = testCase[0]!.GetValue<string>();
                bool tagIsFalse = testCase[1] is JsonValue jv && jv.TryGetValue<bool>(out bool boolVal) && boolVal == false;
                uint? tag = tagIsFalse ? null : (uint)testCase[1]!.GetValue<long>();
                string xAddress = testCase[2]!.GetValue<string>();
            ...
            for (int i = 0; i < testCases.Count; i++)
            {
                ...
                bool tagIsFalse = testCase[1] is JsonValue jv && jv.TryGetValue<bool>(out bool boolVal) && boolVal == false;
                uint? tag = tagIsFalse ? null : (uint)testCase[1]!.GetValue<long>();
                string xAddress = testCase[3]!.GetValue<string>();
