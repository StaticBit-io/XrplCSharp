...
public class VerifiableIntentTests
{
    ...
    private sealed class CapturingInner : HttpMessageHandler
    {
        ...
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage req, CancellationToken ct)
        {
            ...
            PaymentRequiredChallenge body = new() { Accepts = { new PaymentRequirement {
                ...
                Extra = new() { ["invoiceId"] = JsonDocument.Parse("\"inv-test-vi\"").RootElement } } } };
    ...
    [TestMethod]
    public async Task TestUPayloadInvoiceIdIsSetInSignature()
    {
        // Verify that the PAYMENT-SIGNATURE envelope carries payload.invoiceId = the raw invoice id string
        CapturingInner inner = new();
        ...
        Assert.IsNotNull(inner.SeenSignature);
        PaymentSignatureEnvelope env = X402Base64Json.Decode<PaymentSignatureEnvelope>(inner.SeenSignature!);
        Assert.AreEqual("inv-test-vi", env.Payload.InvoiceId,
            "payload.invoiceId must equal the raw invoiceId string from extra");
    }
}
