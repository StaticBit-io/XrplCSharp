...
namespace Xrpl.Tests.Wallet.Tests.Sign
{
    ...
    public class TestUSign
    {
        ...
        JsonNode requestJson = JsonNode.Parse(requestFixture);
        ...
        JsonNode responseJson = JsonNode.Parse(responseFixture);
        ...
        public void TestSignSuccessful()
        {
            ...
            JsonNode some = requestJson["sign"]["normal"];
            SignatureResult result = wallet.Sign(JsonSerializer.Deserialize<Dictionary<string, object>>(some, XrplJsonOptions.Default));
        ...
        {
            var wallet = XrplWallet.FromSeed("ss1x3KLrSvfg7irFc1D929WXZ7z9H");
            JsonNode some = requestJson["sign"]["escrow"];
            SignatureResult result = wallet.Sign(JsonSerializer.Deserialize<Dictionary<string, object>>(some, XrplJsonOptions.Default));
        ...
        {
            var wallet = XrplWallet.FromSeed("ss1x3KLrSvfg7irFc1D929WXZ7z9H");
            JsonNode some = requestJson["sign"]["ticket"];
            SignatureResult result = wallet.Sign(JsonSerializer.Deserialize<Dictionary<string, object>>(some, XrplJsonOptions.Default));
