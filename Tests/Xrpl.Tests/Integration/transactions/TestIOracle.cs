...
public class TestIOracle
{
    ...
    public async Task TestOracleSet_ReadBack_FieldValuesRoundTrip()
    {
        ...
        var response = (await client.AccountObjects(request)).Result;
    ...
    private static async Task<DateTime> GetLedgerCloseTimeAsync()
    {
        ...
        var ledgerResponse = (await client.Ledger(ledgerRequest)).Result;
    ...
    private static async Task<bool> VerifyOracleExists(string account, uint oracleDocumentId)
    {
        ...
        try
        {
            response = (await client.AccountObjects(request)).Result;
        }
        catch (Exception ex)
        {
