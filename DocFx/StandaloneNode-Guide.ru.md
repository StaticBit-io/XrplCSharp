...
- В `develop` rippled переименован в **`xrpld`**, а публикация пакетов в августе 2026 переехала к XRP Ledger Foundation на `packages.xrplf.org`. Канал — `deb-develop`, суита — `any main`, а не codename дистрибутива. Старый `repos.ripple.com` продолжает отвечать, но перестал получать сборки 15.09.2026, поэтому стенд, направленный туда, отслеживает замороженный снимок и при этом выглядит исправным.
- **Пинуйте версию** (`ARG XRPLD_VERSION` в `.ci-config/Dockerfile.nightly`). В новой схеме (`3.5.0~b0-1187.20260921git0229c29`) номера сборок монотонны, поэтому пин нужен не ради порядка, а ради воспроизводимости и ради того, чтобы `generate-amendments.sh` взял ровно тот коммит, из которого собран бинарник: бинарник отвергает неизвестные ему имена амендментов.
