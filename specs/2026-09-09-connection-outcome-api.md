...
Объявления, оставшиеся без поколения, — это те, что говорят от имени клиента целиком, а не одного
перехода: `Connecting` от самого захвата, который его только что взял, пути `Disconnect()`, и
`Connected` из `OnceOpen`.
