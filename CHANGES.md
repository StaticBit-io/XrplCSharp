...
* **`AdminUser`/`AdminPassword` — admin commands over WebSocket** — the mechanism rippled actually accepts for ws/wss: `admin_user`/`admin_password` travel *inside the request JSON*, not in a header. Without them, an admin-credentialed port answers `ledger_accept`, `stop`, `connect` and friends with `noPermission` regardless of the client's IP (rippled's `isAdmin` requires both a matching `admin` net **and** correct credentials). Both must be set for either to be sent, mirroring rippled's own check
