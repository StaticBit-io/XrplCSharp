...
* **MPT path steps (`0x40`)** — `PathSet` only knew the three classic hop-type bits (`0x01` account, `0x10` currency, `0x20` issuer). rippled added `STPathElement::TypeMpt = 0x40` in **3.2.0**, so a hop can now carry a 24-byte `MPTokenIssuanceID` instead of a currency. The gap was silent in both directions: `FromParser` matched none of its masks on a `0x40` byte, produced an empty hop and left the 24 MPTID bytes unread — every following byte was then parsed at the wrong offset — while `SynthesizeType` had no way to emit the bit at all. Now handled end to end:
  * `PathHop.MptIssuanceId` (`Hash192`) with a second constructor, `HasMpt()`, `TypeMpt`/`TypeAll` constants; `currency` and `mpt_issuance_id` in one step throw `InvalidJsonException`, matching rippled, which throws `bad path element: MPT and Currency`
  * serialization order mirrors `STPathSet::add()` — type byte, then account(20), MPTID(24), currency(20), issuer(20)
  * `FromParser` now rejects what rippled rejects: a type byte carrying bits outside `TypeAll` (`0x71`), currency together with MPT, and an empty path (a leading or doubled `0xFF` separator) — previously any garbage byte was accepted and silently mis-parsed, and an empty path survived decoding but disappeared on re-encoding
  ...
  * `Payment.IsPathStep` accepts `mpt_issuance_id` as a valid step asset and rejects it in combination with `currency`
  * `TestUPathSet` pins the layout of both the classic and the MPT hop against rippled's, plus the round trip and both rejection paths
...
* **`Path.TypeHex` is `[Obsolete]`** — rippled removed `type_hex` from `STPath::getJson` in **1.7.0** (commit `f0724694`); only the unused `JSS(type_hex)` declaration survives in `jss.h`. No server has emitted the field for five years, so the property could never be anything but `null`. Verified against mainnet: 19 transactions carrying `Paths` across three consecutive ledgers, 21 path steps, `type` present in all 21 and `type_hex` in none, plus `ripple_path_find` on `s1`/`s2.ripple.com`
* **`Path.Type` is a `[Flags]` enum now** (**breaking**) — the hop type is a bitmask, and both layers spelled it as a bare integer: `Path.Type` was `int?` and `PathHop.Type` was `int`, so callers compared against magic `48`. Both now use `PathStepType` (`Xrpl.BinaryCodec.Enums`), the single enum shared by the model and the codec, matching how ledger objects already type their flags (`AccountRootFlags` and eight more). The wire format is unchanged — `XrplJsonOptions` deliberately registers no global `JsonStringEnumConverter` because XRPL protocol enums are numeric — and a value carrying a bit the enum does not declare survives deserialization untouched, which `TestUPathStep` pins along with the numeric wire form. The one behavioural loss: `"type":"48"` sent as a *string* no longer parses, since `NumberHandling.AllowReadingFromString` does not apply to enums; rippled always sends it as a number
