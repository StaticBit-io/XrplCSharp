...
| Response envelope | `BaseResponse.Result` (`object`) | `RawResult` (bytes as sent) and `ResultSlice` (bounds) |
...
Sugar methods (`GetXrpBalance`, `GetLedgerIndex`, `SubmitAndWait`, `GetOrderBook`, `Submit`, `Autofill`) keep their signatures — they return computed values, not node responses.
