...
- **Assets:** XRP and RLUSD/IOU are supported, exercised end-to-end against a standalone `rippled`.
- **Schemes:** only `exact` is implemented — which is the **only** scheme the t54 facilitator advertises for XRPL (`GET /supported` → `{scheme:"exact", network:"xrpl:1"}`). The scheme is matched explicitly and the wire types are isolated, so other schemes can be added later, but none exist for XRPL today.
- **Wire-format fidelity:** the request/response shapes match the **live t54 facilitator OpenAPI 1:1** — our `PaymentSignatureEnvelope` = t54 `PaymentPayload`, our `PaymentRequirement` = t54 `PaymentRequirements`, our `PaymentResponseEnvelope` = t54 settle response. Verified against the t54 docs and the `mpcp-protocol/x402-xrpl` conformance adapter.
- **Live t54 settlement (known issue):** `T54Facilitator` ships and is wire-correct, but the **t54 testnet facilitator currently rejects `/settle` with `verify_failed:invalid_payload`** at its own signature-verification stage — even though the same signed transaction is accepted on-ledger with `tesSUCCESS`. This appears to be a t54-side issue (their Python `xrpl-py` verification), not a wire-format mismatch on our side. The live interop test (`X402T54LiveInteropTests`) is `[Ignore]`-by-default; confirm with t54 before relying on their hosted facilitator. The payment-id field names are configurable (see *Interop notes*) should a target facilitator expect different naming.
- **Verifiable Intent:** a **passthrough** is provided — set `X402ClientOptions.VerifiableIntentProvider` (an `IVerifiableIntentProvider`) and its `extensions` object is attached to each PAYMENT-SIGNATURE under `x402Secure.verifiableIntentChain`. The full SD-JWT L1→L3 credential chain (Mastercard Agentic / Trustline) is **not** generated by this package — supply it from your own provider.
