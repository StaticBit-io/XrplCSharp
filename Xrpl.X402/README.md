...
- **Live t54 interop — CONFIRMED:** the client's PAYMENT-SIGNATURE is accepted by the **live t54 testnet facilitator** — `/verify` returns `isValid:true` and `/settle` settles on-chain (verified with a real testnet transaction). Matching t54 requires: `payload` carrying `invoiceId`; `Payment.InvoiceID = SHA-256(invoiceId)`; a `MemoData` = hex(invoiceId); and the requirement's `extra.sourceTag` (`804681468`) stamped on the tx (t54 enforces SourceTag). `T54Facilitator` calls the real `/verify` + `/settle`. The live test `X402T54LiveInteropTests` is `[Ignore]`-by-default (it depends on the public testnet faucet and the t54 hosted facilitator), but has been run green.
